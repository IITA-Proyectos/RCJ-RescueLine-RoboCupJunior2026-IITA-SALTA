...
lower_green = np.array([86,102, 110])  # lab
upper_green = np.array([161,114, 135])
lower_silver_hsv = np.array([95, 95, 85])  # Valores en espacio HSV para detectar el plateado
upper_silver_hsv = np.array([134, 140, 130])  # Ajusta estos valores si es necesario
last_angles = []
...
def modo_rescate():
    ...
    last_target_box = None  # inicializamos antes de usar
    global is_stopped  # <--- agregar esta línea
    global estado, ser
    # flag local para solicitar salida del modo rescate
    stop_rescate = False
    ...
    # ---- CONFIG ----
    MODEL_PATH = "/home/iita/Desktop/best.onnx"
    CLASS_NAMES = ['Silver Ball','Black Ball']
    ...
    # --- helper: MOSSE maker + CentroidTracker fallback (sin cambios) ---
    def make_mosse():
        try:
            if hasattr(cv2, "legacy") and hasattr(cv2.legacy, "TrackerMOSSE_create"):
    ...
    # --- capture + infer threads (prácticamente iguales a los tuyos) ---
    def capture_thread():
    ...
    CLASS_THRESH = {0: 0.5, 1: 0.5}
    CLASS_COLORS = {0: (0, 0, 0), 1: (192, 192, 192)}
    ...
    def select_target_from_list(boxes):
        targets = {'Silver Ball': [], 'Black Ball': []}
        for d in boxes:
            cls_id = d['cls']
            if cls_id == 1:
                targets['Silver Ball'].append(d)
            elif cls_id == 0:
                targets['Black Ball'].append(d)
        for k in ('Silver Ball','Black Ball'):
            if targets[k]:
                return targets[k][0]
        return None
    ...
    # --- SERIAL MONITOR local (solo durante modo_rescate) ---
    ...
    # ---- main_loop con checks periódicos de stop_rescate ----
    def main_loop():
        global last_target_box
        global is_stopped
        processed = 0
        ...
        while True:
            # CHECK PARA SALIR si serial pidió stop
            if stop_rescate:
                print("main_loop: stop_rescate activo -> saliendo de rescate")
                break

            try:
                item = result_q.get(timeout=0.25)  # timeout para poder chequear la flag periódicamente
            except queue.Empty:
                continue
...
while True:
    ...
    while estado == 'linea':
        ...
        for contour in silver_contours:
            ...
            print(area)
            if area > 100 :  # Define un umbral para el ÃƒÆ’Ã‚Â¡rea para evitar ruido
                silver_line = True
                break
        output = [255, speed,
                  254, round(angle) + 90 ,
